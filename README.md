...
GitHub Pages veya Netlify'de statik olarak yayınlanabilen bir PWA scaffold'u. Tek mağaza/showroom
için Personel, Müdür ve Admin rolleri; görev yönetimi, akıllı formlar (ciro/stok/denetim), stok
kataloğu ve offline senkron altyapısı hazır durumda.

## Şu an ne çalışıyor, ne çalışmıyor

**Hazır ve çalışıyor:**
- Tüm sayfalar ve rol bazlı navigasyon (Personel / Müdür / Admin)
...
1. Gerçek bir Firebase projesi + Firestore güvenlik kuralları
2. Firestore `profiles` koleksiyonu ile giriş yapan kullanıcının gerçek rolünü çekme (bkz.
   "Firebase kurulumu" bölümü — bunu her yeni kullanıcı için elle yapman gerekiyor)
3. Fotoğrafların Firebase Storage'a taşınması (şu an sıkıştırılmış base64 olarak saklanıyor,
   çalışıyor ama Storage daha doğru uzun vadeli çözüm)
4. Push bildirimleri (altyapı yok; aşağıda "Push Bildirimler" bölümüne bak)
5. GitHub Pages kullanacaksan repo adını `next.config.js` / workflow dosyasındaki
...
| Koleksiyon | Ne için |
|---|---|
| `profiles` | Kullanıcı uid'sine göre `adSoyad`, `rol` |
